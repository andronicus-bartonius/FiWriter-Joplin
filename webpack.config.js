...
for (const moduleName of builtinModules) {
	...
	moduleFallback[`node:${moduleName}`] = false;
}
...
function resolveWebviewScriptPath(name) {
	...
	if (!fs.pathExistsSync(fullPath)) throw new Error(`Could not find webview script: "${name}" at "${fullPath}"`);
...
function buildWebviewConfigs(userConfig) {
	if (!userConfig.webviewScripts || !userConfig.webviewScripts.length) return [];
...
function main(environ) {
	...
	const configs = {
		...
		// Builds webview scripts (Preact TSX) as browser-compatible bundles.
		buildWebviews: buildWebviewConfigs(userConfig),
